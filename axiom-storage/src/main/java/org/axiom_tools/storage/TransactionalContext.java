...
public class TransactionalContext {
	...
	int count(final Class<ItemType> clazz) {
		...
		return countWith(new Callable<Integer>() {
			@Override public Integer call() throws Exception { return countAll(clazz); }
		});
	}
	...
	ItemType findItem(final ItemType item) {
		...
		return commitWith(new Callable<ItemType>() {
			@Override public ItemType call() throws Exception { return read(item); }
		});
	}
	...
	ItemType findItem(final ItemType item) {
		...
		return commitWith(new Callable<ItemType>() {
			@Override public ItemType call() throws Exception { return find(item); }
		});
	}
     ...
     */
    public <ItemType extends SurrogatedItem>
    List<ItemType> findItems(final QueryBuilder builder) {
		return commitWith(new Callable<List<ItemType>>() {
			@Override public List<ItemType> call() throws Exception {
                return builder.buildQuery(manager).getResultList();
            }
		});
    }
	...
	ItemType useItem(final ItemType item, final SurrogatedItem.Usage usage) {
		...
		if (item == null || item.getKey() == 0) return null;
		return commitWith(new Callable<ItemType>() {
			@Override public ItemType call() throws Exception { 
	...
	ItemType saveItem(final ItemType item) {
		return commitWith(new Callable<ItemType>() {
			@Override public ItemType call() throws Exception { return save(item); }
		});
	}
	...
	int countMapComponents(final ItemType item, final int mapIndex) {
		return commitWith(new Callable<Integer>() {
			@Override public Integer call() throws Exception { return countMapElements(item, mapIndex); }
		});
	}
	...
	ResultType getMapComponent(final ItemType item, final int mapIndex, final Object key) {
		return commitWith(new Callable<ResultType>() {
			@Override public ResultType call() throws Exception { return getMapElement(item, mapIndex, key); }
		});
	}
	...
	boolean removeItem(final ItemType item) {
		...
		return commitWith(new Callable<Boolean>() {
			@Override public Boolean call() throws Exception { return remove(item); }
		});
	}
	...
	void saveComponents(Set<ItemType> components) throws Exception {
		HashSet<ItemType> results = new HashSet<>(components);
